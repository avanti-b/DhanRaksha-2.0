...
async function loadSample() {
  try {
    ...
    if (!samples.length) {
      $("s-note").textContent =
        "No samples available here — enter values manually and submit.";
      return;
    }
    ...
    // The true label is shown so you can compare the model's answer against
    // reality. It is never sent to the API — the model scores this row blind.
    $("s-note").textContent =
      `Loaded a real transaction whose true label is ` +
      `${sample.label.replace("known_", "").replace("_", " ")}. ` +
      `Submit it to see what the model says.`;
    $("p-result").style.display = "none";
  } catch (error) {

...
from backend.azure_integration.telemetry import track_prediction
...
logger = logging.getLogger("dhanraksha.fraud")
...
class FraudService:
    def __init__(
        ...
        # Optional Azure Storage Queue producer. None locally, which keeps the
        # flow entirely synchronous.
    ...
    def predict(self, payload: dict) -> dict:
        ...
        # Hand off to asynchronous enrichment. Best effort by design: the
        # decision is already scored, persisted and about to be returned, so a
        # queue outage must not fail the request.
        queued = False
        if self.queue_client is not None:
            queued = self.queue_client.enqueue(
                {
        ...
        return {
            ...
            "timestamp": record["timestamp"],
            "case_id": case["case_id"] if case else None,
            "queued_for_async_processing": queued,
